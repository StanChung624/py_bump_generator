...
def _extract_circles_auto(
    dxf_path: str,
    *,
    require_closed: bool,
    min_points: int,
    max_rms: float,
) -> Tuple[str, List[CircleFit]]:
    circles_insert = extract_circles_from_inserts(
        dxf_path,
        require_closed=require_closed,
        min_points=min_points,
        max_rms=max_rms,
    ...
    if circles_insert:
        return "INSERT", circles_insert

    polylines = parse_dxf_polylines(dxf_path)
    circles_poly = extract_circles_from_polylines(
        polylines,
        require_closed=require_closed,
        min_points=min_points,
        max_rms=max_rms,
    )
    return "POLYLINE", circles_poly


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Extract circle centers/radii from DXF.")
    parser.add_argument("dxf_path", help="Path to .dxf file")
    parser.add_argument("--method", choices=("auto", "insert", "polyline"), default="auto")
    parser.add_argument("--min-points", type=int, default=6)
    parser.add_argument("--max-rms", type=float, default=1e-2)
    parser.add_argument("--allow-open", action="store_true", help="Allow open polylines")
    parser.add_argument("--print-limit", type=int, default=100, help="Max rows to print; 0 prints all")
    parser.add_argument("--csv", default="", help="Optional output CSV path")
    parser.add_argument("--vtp", default="", help="Optional output VTP path")
    parser.add_argument("--unit-scale", type=float, default=0.001, help="Coordinate/radius scale before export")
    ...
    if args.method == "insert":
        ...
        method_used = "INSERT"
    elif args.method == "polyline":
        polylines = parse_dxf_polylines(args.dxf_path)
        circles = extract_circles_from_polylines(
            polylines,
            require_closed=require_closed,
            min_points=args.min_points,
            max_rms=args.max_rms,
        )
        method_used = "POLYLINE"
    else:
        method_used, circles = _extract_circles_auto(
            args.dxf_path,
            require_closed=require_closed,
            min_points=args.min_points,
            max_rms=args.max_rms,
    ...
    print(f"Found {len(circles)} circles (via {method_used})")
    limit = args.print_limit
    rows = circles if limit == 0 else circles[: max(limit, 0)]
    for i, c in enumerate(rows, start=1):
        print(
            f"[{i}] layer={c.layer} center=({c.cx:.9f}, {c.cy:.9f}) "
            f"r={c.r:.9f} rms={c.rms:.3e} npts={c.npts}"
        )
    if limit > 0 and len(circles) > limit:
        print(f"... truncated {len(circles) - limit} rows (use --print-limit 0 for all)")

    if args.csv or args.vtp:
        vbumps: List[VBump] = []
        for c in circles:
            vbumps.append(VBump(
                x0=c.cx * args.unit_scale, y0=c.cy * args.unit_scale, z0=0,
                x1=c.cx * args.unit_scale, y1=c.cy * args.unit_scale, z1=10,
                D=c.r * 2 * args.unit_scale,
            ))

        if args.vtp:
            write_vbumps_vtp(vbumps, args.vtp)
        if args.csv:
            to_csv(args.csv, vbumps)
